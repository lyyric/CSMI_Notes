...
SELECT SOCIETE, NO_COMMANDE, COUNT( REF_PRODUIT)
FROM CLIENTS NATURAL JOIN COMMANDES
JOIN DETAILS_COMMANDES USING(NO_COMMANDE)
GROUP BY SOCIETE, NO_COMMANDE
HAVING COUNT( REF_PRODUIT) >= ( SELECT COUNT( REF_PRODUIT)
FROM COMMANDES NATURAL JOIN
DETAILS_COMMANDES
WHERE NO_COMMANDE = 216798);

SELECT SOCIETE, NO_COMMANDE
FROM CLIENTS JOIN COMMANDES USING ( CODE_CLIENT )
JOIN DETAILS_COMMANDES USING ( NO_COMMANDE )
JOIN PRODUITS USING ( REF_PRODUIT )
WHERE NOM_PRODUIT = 'Chai'
AND PAYS = 'France'
AND CODE_CLIENT IN (SELECT CODE_CLIENT
