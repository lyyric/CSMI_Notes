...
1. **Si $A$ est normale**, c'est-à-dire $A^*A = AA^*$ (resp. $\bar{A}^T A = A \bar{A}^T$), alors $A$ est diagonalisable, donc il existe une base orthonormée $\exists P \in U_n(\mathbb{C})$ (resp. $P \in O_n(\mathbb{R})$), telle que :
$$
...
2. **Soit** $\| \cdot \|$ **une norme sur** $\mathbb{C}^n$. On lui associe une norme dite subordonnée $\| \cdot \|_M$ sur $M_n(\mathbb{C})$
$$
   \|A\|_M = \sup_{x \neq 0} \frac{\|Ax\|}{\|x\|}
$$
   Elle vérifie :
...
#### Propriété des Matrices Hermitiennes
