...
Rappel: on a vu que l'on peut multiplier 2 polynômes en $O(n^{1 + \epsilon})$, où $\epsilon > 0$ est arbitraire.
...
(i) $\rightarrow$ coeffs: $P(x) = a_0 + a_1 x + ... + a_{n-1} x^{n-1}$
...
$1 + 2 + ... + n = \frac{n(n+1)}{2} \in \Theta(n^2)$

Note: $\theta(f(n)) \geq C f(n)$
...
2. **$O(g)$** : On note $f = O(g)$ s'il existe $c_1, c_2 > 0$ tels que $c_1 \cdot g(n) \leq f(n) \leq c_2 \cdot g(n)$ pour tout $n$.


**Remarque**: Si $f = O(g)$ et $g = O(f)$, alors $f = O(g)$.

Montrer que:

(i) $C(n) = O\left(n^{\log_b a}\right)$ si $f(n) = O\left(n^{\log_b a - \epsilon}\right)$ avec $\epsilon > 0$.

(ii) $C(n) = O\left(n^{\log_b a} \cdot \log n\right)$ si $f(n) = O\left(n^{\log_b a}\right)$.

(iii) $C(n) = O(f(n))$ s'il existe $\epsilon > 0$ tel que $f(n) = \Omega\left(n^{\log_b a + \epsilon}\right)$ et s'il existe $c < 1$ et $n_0$ tel que $a \cdot f(n/b) \leq c \cdot f(n)$ pour tout $n \geq n_0$.
